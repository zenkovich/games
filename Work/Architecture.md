...
- `Core/WordBoardMotion.{h,cpp}` — логическая модель движения плиток при обвале:
  офсеты клеток в клетках, каскад стартов снизу вверх, колонка-стек (плитка не
  опускается ниже соседки — наложения исключены), спавны входят из-за верхней
  границы скрытыми (hidden). Сервис владеет и обновляет; вью только читает
  офсеты/видимость и рисует.
...
- `Tile.proto` — плашка буквы поля (слои back/sel/ice/stone/letter/points +
  крупные бонусы bomb/rocket/fireworks по центру — бонус занимает слот);
...
- `FxFlash/FxGlow/FxBeam.proto` — виджеты эффектов хореографии очков;
- `FxRocket.proto` — ракета бонуса: FlightTrajectoryComponent + анимация полёта
  (крен, искровый след) + три цветных салюта суб-треками на прилёте (пул ×10
  для залпа фейерверка);
